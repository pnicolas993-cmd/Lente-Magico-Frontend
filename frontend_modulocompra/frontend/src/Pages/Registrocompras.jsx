...
function RegistroCompras() {
    ...
    const handleEliminarLineaNuevo = (index) =>
        setFormNuevo({
            ...formNuevo,
            detalle: formNuevo.detalle.filter((_, i) => i !== index)
        });

    const handleAgregar = async () => {
        try {
            if (!formNuevo.idProveedor) {
                alert("Selecciona un proveedor.");
                return;
            }

            if (!formNuevo.fechaCompra) {
                alert("Selecciona la fecha de compra.");
                return;
            }

            if (!formNuevo.numComprobante.trim()) {
                alert("Ingresa el número de comprobante.");
                return;
            }

            if (!formNuevo.detalle.length) {
                alert("Agrega al menos un producto.");
                return;
            }

            for (const linea of formNuevo.detalle) {
                if (!linea.nombreProducto.trim()) {
                    alert("Todos los productos deben tener un nombre.");
                    return;
                }

                if (!linea.idCategoria) {
                    alert(
                        `Selecciona una categoría para el producto "${linea.nombreProducto}".`
                    );
                    return;
                }
                ...
                if (!linea.cantidad || Number(linea.cantidad) <= 0) {
                    alert(
                        `La cantidad del producto "${linea.nombreProducto}" debe ser mayor que 0.`
                    );
                    return;
                }
                ...
                if (
                    ...
                    alert(
                        `Ingresa un costo válido para "${linea.nombreProducto}".`
                    );
                    return;
                }
            }
            ...
            if (!response.ok) {
                throw new Error(
                    resultado.error || 'Error al crear la compra'
                );
            }
                        ...
                        <div className="row g-3">
                            ...
                            <div className="col-md-6">
                                ...
                                <small
                                    ...
                                    style={{ fontSize: '11px' }}
                                >
                                    El inventario se actualiza según
                                    la lógica definida en el backend
                                    para cada estado.
                                </small>
                            </div>
                            ...
                            <div className="col-12">
                                ...
                                <button
                                    ...
                                    onClick={handleAgregarLineaNuevo}
                                >
                                    + Agregar producto
                                </button>
